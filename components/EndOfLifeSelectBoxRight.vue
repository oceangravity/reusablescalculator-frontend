<template>
  <select-box :options="store.endOfLifeSelectBox" @select="select">
    <template #button="{ open, close, selected }">
      <button @mousedown="open" @blur="close" class="z-10 w-full flex jw-full cursor-pointer border bg-white focus:ring-2 focus:outline-none focus:ring-blue-300 rounded text-sm px-4 py-2.5 text-center inline-flex items-center" type="button">
...
const select = (selected: ItemList) => {
  store.calculator.singleUseContainer.endOfLifeSelectBox = selected
}
</script>
